...
```bash
bash scripts/build_all.sh        # runs missions 00 → 06 headlessly, logs to docs/evidence/raw/
```
...
| `prompts/06_app.md` | Streamlit Command Center (Executive OEE, Alert Triage, Asset Digital Twin, Ask Aegis, Work-Order Review) | `app/` | 5 pages render, approval flow works |
...
| `scripts/` | Harness: `preflight_probes`, `build_all`, `inject_anomaly`, `demo_reset`, `capture_evidence`, `with_automations` |
...
| `docs/coco-evidence.md` | How to verify CoCo drove planning, development, execution, and testing |
